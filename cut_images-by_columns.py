...
def cut_images_in_fldr(D_image, roi, image_path, cal_path):
    ...
    def col_imgs2sql(rowcol, names, segmented_dict,addition=0):  # rowcol = keys of segmented images, names = name of plant
        df_collector = {}

        for key1, name in zip(rowcol, names):
            print(key1, name)
            print('begining img2sql', time.strftime("%H:%M:%S"))
            print(segmented_dict[key1][1])
            print (np.array(segmented_dict[key1][1]).max())
            print (segmented_dict[key1][1].shape)
            print(segmented_dict[key1][0])
            print(np.array(segmented_dict[key1][0]).max())
            print (segmented_dict[key1][0].shape)
            print(diff.shape)
            ys = list(map(lambda x:x-addition if x-addition > 0 else 0, segmented_dict[key1][0]))
            loop_DIFF = diff[segmented_dict[key1][1], ys] # only diff pixels in inside the polygon
            ...
            date = metadata['acquisition date'].split(' ')[1]
            time_aquision = metadata['start time'].split(' ')[2]
            ...
            plant_pixels = image[segmented_dict[key1][1], ys][plant_idx[0]]
            plant_pixels_spectra = pd.DataFrame(columns=metadata['wavelength'], data=plant_pixels.tolist())
            ...
            plant_pixels = rad_image[segmented_dict[key1][1], ys][plant_idx[0]]
            ...
            plant_pixels = refle_img[segmented_dict[key1][1], ys][plant_idx[0]]
    ...
    def calibration_sql(name, data):
        ...
        temp_dict3 = pd.DataFrame(data.tolist(), metadata['wavelength'])
    ...
    column_dicts = [first_column, second_column, third_column, forth_column]
    ...
    third_column_names = ['1C', '2C', '3C', '4C', '5C', '6C', '7C', '8C', '9C', '10C', '11C', '12C', '13C', '14C',
                          '15C',
                          '16C', '17C', '18C']
    columns_names = [first_column_names,second_column_names, third_column_names, forth_column_names]
    ...
    keys = [first_keys,second_keys,third_keys,forth_keys]
    ...
    # black_y_stop = df[df['group_id'] == 'Black1']['Y'].max()
        ...
        # calibrate the image
    image = envi.open(image_path)
    metadata = image.metadata
    cal_img = envi.open(cal_path + '.hdr', image=cal_path + '.cal').load().reshape(1024, 448)
    print (cal_img.shape)
    ...
    image = image[ref_y_start:ref_y_stop, ref_x_start:ref_x_stop]


    rad_image = np.empty_like(image,dtype='float')
    for i in range(image.shape[0]):
        rad_image[i] = np.divide(np.multiply(np.subtract(image[i], dark_img[ref_x_start:ref_x_stop]), cal_img[ref_x_start:ref_x_stop]), time_integration)

    # calculate reflectance
    # white panel mean spectrum
    spectralon = rad_image
    ...
    # now we need to load 4 rows at a time

    xstart,ystart = 0,0
    for ver in range(4):
        print ('ver is: ', ver)
        if ver != 3:
            line = df[df['group_id'] == ver_lines[ver]]['coor'].tolist()
            top_of_line = line[0]
            top_of_line_x, top_of_line_y = top_of_line[0],top_of_line[1]
            # print(top_of_line_x, top_of_line_y)
            bottom_of_line = line[-1]
            bottom_of_line_x,bottom_of_line_y = bottom_of_line[0],bottom_of_line[1]
            line_max = np.array(line)[:,0].max()
            # print(bottom_of_line_x, bottom_of_line_y)
            if bottom_of_line_x > top_of_line_x:
                image = envi.open(image_path)[:, ystart:line_max+1]
                rad_image = np.empty_like(image,dtype='float')
                for i in range(image.shape[0]):
                    rad_image[i] = np.divide(np.multiply(np.subtract(image[i], dark_img[ystart:line_max+1]), cal_img[ystart:line_max+1]),
                                             time_integration)
                ystart = top_of_line_x
                addition = line_max
            else:
                image = envi.open(image_path)[:, ystart:line_max+1]
                rad_image = np.empty_like(image,dtype='float')
            ...
            red_wave = float(metadata['wavelength'][RED_CHANNEL])
            nir = refle_img[:, :, NIR_CHANNEL]
            nir_wave = float(metadata['wavelength'][NIR_CHANNEL])
            ...
            if ver == 0:
                ...
                addition1 = addition
                second_column_df = col_imgs2sql(second_keys, second_column_names, second_column, addition0)
            else:
                addition2 = addition
                third_column_df = col_imgs2sql(third_keys, third_column_names, third_column, addition1)
        else:
            ystart = ystart
            image = envi.open(image_path)[:, ystart:]
            rad_image = np.empty_like(image,dtype='float')
            for i in range(image.shape[0]):
                rad_image[i] = np.divide(np.multiply(np.subtract(image[i], dark_img[ystart:]), cal_img[ystart:]),
                                         time_integration)
            print (image.shape)
            refle_img = np.divide(rad_image, spectralon_avg)
            ...
            print("calculated diff image", lineno())
            # addition =
            forth_column_df = col_imgs2sql(forth_keys, forth_column_names, forth_column, addition2)
    ...
    df2.to_pickle(roi[:-4] + 'pickle',protocol = 4)
